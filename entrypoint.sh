...
# Allow checksum to work even if file doesn't exist yet
if [ -f "$RESOLVED_PATH" ]; then
  CHECKSUM=$(shasum "$RESOLVED_PATH")
  echo "Identified Package.resolved at '$RESOLVED_PATH'."
  echo "Checksum: $CHECKSUM."
else
  echo "No existing Package.resolved found. Will generate a new one."
  CHECKSUM=""
fi
...
xcodebuild ${xcodebuildInputs} -resolvePackageDependencies -disablePackageRepositoryCache $xcodebuildDestination
echo "::endgroup"

# Re-check Package.resolved (may have just been generated)
if [ -f "$RESOLVED_PATH" ]; then
  NEWCHECKSUM=$(shasum "$RESOLVED_PATH")
else
  echo "::error::Package.resolved was not generated by xcodebuild!"
  exit 1
fi
